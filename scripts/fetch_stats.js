...
async function fetchStats() {
  ...
  try {
    ...
    const userRes = await fetch(`https://api.github.com/users/${username}`, {
      headers: { "User-Agent": "readme-aura-fetcher" }
    });
    ...
    const searchRes = await fetch(`https://api.github.com/search/commits?q=author:${username}`, {
      headers: { 
        "User-Agent": "readme-aura-fetcher",
    ...
    // Fallback to safe defaults if network request fails
    const sourcePath = path.join(__dirname, '..', 'readme.source.md');
    let content = fs.readFileSync(sourcePath, 'utf8');
    content = content.replace('{{REPOS}}', "8");
    content = content.replace('{{ACTIVE_DAYS}}', "592");
    content = content.replace('{{COMMITS}}', "51");
    fs.writeFileSync(sourcePath, content, 'utf8');
    console.log("Fell back to default stats in readme.source.md");
  }
}
